...
//                    '</span></span>';
    ...
    var embedAnnotation = function(elem, parse, cdata, data, options) {
        ...
        elem.show();
        var ttt = $('<div class="ttt" data-embednum="' + myIdSeq + '">');
        var lines = data.split(/\n/);
        for (var i=0; i<lines.length; i++) {
          if (lines[i].startsWith("T")) {
            var parts = lines[i].split(/\s+/);
            var wordid = myIdSeq + '-' + i.toString();
            var lemma = parts[parts.length-1];
            var radio = '<input type="radio" id="' + wordid + '" name="ttt-' + myIdSeq + '" value="' + lemma + '">';
            var label = ' <label for="' + wordid + '">' + lemma + '</label>';
            ttt.append(radio + label);
          }
        }
        ttt.insertAfter($(elem));
    };
