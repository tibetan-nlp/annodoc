...
//                    '</span></span>';
    ...
    var embedAnnotation = function(elem, parse, cdata, data, options) {
        ...
        elem.show();
        var prefixedId = "ttt-" + myIdSeq;
        var lines = data.split(/\n/);
        var $tttWrapper = $('<div class="ttt-wrapper">');
        var $radios = $('<div class="ttt-radios">');
        var wordCount = 0;
        for (var i=0; i<lines.length; i++) {
          if (lines[i].startsWith("T")) {
            wordCount++;
            var parts = lines[i].split(/\s+/);
            var wordId = 'ttt-' + myIdSeq + '-' + wordCount.toString();
            var lemma = parts[parts.length-1];
            if (lemma.indexOf('√') !== -1) {
              lemma = lemma.substring(0, lemma.indexOf('√'));
            }
            var radio = '<input type="radio" id="' + wordId + '" name="' + prefixedId + '" value="' + lemma + '">';
            var label = '<label for="' + wordId + '">' + lemma + '</label>';
            $radios.append(radio + label);
          }
        }
        $tttWrapper.append($radios);
        $tttWrapper.append($('<div class="ttt-response" id="' + prefixedId +'">'));
        $(elem).parent().after($tttWrapper);
        $('input[type="radio"][name="' + prefixedId + '"]').change(function() {
          console.log('changed');
          var radioValue = $('input[name="' + prefixedId + '"]:checked').val();
          var tttUrl = "https://tibetan-nlp.github.io/lim-annodoc/static/ttt/" + radioValue + ".json";
          $('#' + prefixedId).html('');
          jQuery.getJSON(tttUrl, function(data) {
            var k = Object.keys(data.words).length;
            var includeWord = k > 1 ? true : false;
            var output = "";
            for (word in data.words) {
              if (data.words.hasOwnProperty(word)) {
                ...
                output += '<ul class="ttt-defs">';
                var defs = data.words[word];
                for (var j=0; j<defs.length; j+=2) {
                  output += "<li><strong>[" + defs[j] + "]</strong> " + defs[j+1] + "</li>";
                }
                output += '</ul>';
              }
            }
            $('#' + prefixedId).html(output);
          });
        });
        $('#ttt-' + myIdSeq + '-1').prop('checked', true).change();
    };
