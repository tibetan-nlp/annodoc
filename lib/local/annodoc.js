...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    var displayComment = function(evt, target, comment, commentText) {
        ...
        adjustToCursor(evt, commentPopup, 50, false, false);
    ...
    var displaySpanInformation = function(evt, target, spanId, spanType,
        ...
        var tttUrl = "https://tibetan-nlp.github.io/lim-annodoc/static/ttt/" + spanText + ".json";
        jQuery.getJSON(tttUrl, function(data) {
          comment += "<dl>";
          for (word in data.words) {
            if (data.words.hasOwnProperty(word)) {
              comment += "<dt>" + word + "</dt>";
              comment += "<dd>" + data.words[word].toString() + "</dd>";
            }
          }
          comment += "</dl>";
          comment += '</div>';
    ...
    var embedAnnotation = function(elem, parse, cdata, data, options) {
        ...
        for (var i=0; i<lines.length; i++) {
          if (lines[i].startsWith("T")) {
            ...
            var wordId = myIdSeq + '-' + (i+1).toString();
            ...
            $tttWrapper.append(radio + label);
          }
        }
        $tttWrapper.append($('<div id="' + prefixedId +'">'));
        $(elem).after($tttWrapper);
        $('input[type="radio"][name="' + prefixedId + '"]').change(function() {
          ...
          jQuery.getJSON(tttUrl, function(data) {
            ...
            for (word in data.words) {
              if (data.words.hasOwnProperty(word)) {
                defs += "<dt>" + word + "</dt>";
                defs += "<dd>" + data.words[word].toString() + "</dd>";
              }
            }
            defs += "</dl>";
            $('#' + prefixedId).html(defs);
          });
        });
    };
